...
describe("AuthMetadata", function () {
  ...
  it("#should authenticate and  serialize and deserialize into JSON seamlessly (backward compatibility)", async function () {
    const instance1 = AuthMetadata.fromJSON({ ...JSON.parse(mpcBackwardCompatibilityTestJson), legacyMetadataFlag: false });
    const instance2 = AuthMetadata.fromJSON({ ...JSON.parse(mpcBackwardCompatibilityTestJson), legacyMetadataFlag: true });
