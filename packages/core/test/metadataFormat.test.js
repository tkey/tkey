...
const mpcBackwardCompatibilityTestJson =
...
const mpcBackwardCompatibilityTestJson1 =
...
describe("Metadata new Formatting tests", function () {
  ...
  it("#should able to deserialize new JSON format with backwardcompability test", async function () {
    const instance = LegacyMetadata.fromJSON(JSON.parse(mpcBackwardCompatibilityTestJson));
    const instanceSerialized = instance.toJSON();
    ...
    equal(stringify(instanceSerialized), mpcBackwardCompatibilityTestJson);

    // try to deserialize using new metadata format
    const instance1 = Metadata.fromJSON(JSON.parse(mpcBackwardCompatibilityTestJson));
    const instance1Serialized = instance1.toJSON();
  ...
  it("#should able to deserialize new JSON format with backwardcompability1 test", async function () {
    const instance = LegacyMetadata.fromJSON(JSON.parse(mpcBackwardCompatibilityTestJson1));
    const instanceSerialized = instance.toJSON();

    // delete for matching purpose ( jsonstring provide do no have tssKeyTypes.default keytype)
    delete instanceSerialized.tssKeyTypes;
    equal(stringify(instanceSerialized), mpcBackwardCompatibilityTestJson1);

    // try to deserialize using new metadata format
    const instance1 = Metadata.fromJSON(JSON.parse(mpcBackwardCompatibilityTestJson1));
    const instance1Serialized = instance1.toJSON();
