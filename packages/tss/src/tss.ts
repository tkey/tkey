...
import { dotProduct, getLagrangeCoeffs, kCombinations } from "@tkey/core";
...
export class TSSModule implements IModule {
  ...
  async setModuleReferences(api: ITKeyApi): Promise<void> {
    this.tbSDK = api;
  }

  async initalizeWithTss(
    tkey: ITKeyApi,
    params: {
  ...
  getTSSCommits(tkey: ITKeyApi): Point[] {
   ...
   */
  async getTSSShare(tkey: ITKeyApi, factorKey: BN, opts?: { threshold: number }): Promise<{ tssIndex: number; tssShare: BN }> {
  ...
  getFactorEncs(tkey: ITKeyApi, factorPub: Point): FactorEnc {
    const metadata = tkey.getMetadata();
  ...
  async _initializeNewTSSKey(tkey: ITKeyApi, tssTag: string, deviceTSSShare, factorPub, deviceTSSIndex?): Promise<InitializeNewTSSKeyResult> {
    let tss2: BN;
    ...
    const tss1Pub = await tkey.getServiceProvider().getTSSPubKey(tssTag, 0);
  ...
  async _refreshTSSShares(
    ...
    const newTSSServerPub = await tkey.getServiceProvider().getTSSPubKey(this.tssTag, tssNonce + 1);
  ...
  async _getRssNodeDetails(tkey: ITKeyApi): Promise<{ serverEndpoints: string[]; serverPubKeys: PointHex[]; serverThreshold: number }> {
