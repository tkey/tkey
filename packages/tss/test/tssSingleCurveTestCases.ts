...
import { factorKeyCurve, TSS_TAG_DEFAULT } from "../src/tss";
...
const singleCurveTestCases = (params: { TSS_KEY_TYPE: KeyType; legacyFlag: boolean; spSecp256k1: boolean }) => {
  ...
  describe(`TSS Single Curve tests, keyType=${TSS_KEY_TYPE}, legacyMetadata=${legacyFlag}, spSecp256k1=${spSecp256k1}`, function () {
    ...
    it(`#should be able to import a tss key for existing account, manualSync=${manualSync}`, async function () {
      ...
      // after import, tkey tss instance is on tssTag = "imported"
    ...
    it(`#should be able to unsafe export final tss key, manualSync=${manualSync}`, async function () {
      ...
      tb.initializeTss({
        factorPub,
        deviceTSSIndex: importedIndex,
        tssKeyType: TSS_KEY_TYPE,
        serverOpts: { authSignatures: signatures },
      });
      await tb.importTssKey(
        {
          tssTag: "imported",
...
const LegacyMetadataFlag = [true, false];

// testcases with secp256k1 service provider with single tss curve
LegacyMetadataFlag.forEach((legacyFlag) => {
  TEST_KEY_TYPES.forEach((TSS_KEY_TYPE) => {
    singleCurveTestCases({
      TSS_KEY_TYPE,
      legacyFlag,
      spSecp256k1: true,
    });
