...
export const sharedTssTests = (mode, torusSP, storageLayer) => {
  ...
  describe.only("TSS tests", function () {
    ...
    describe("with tss, tkey share deletion", function () {
      ...
      before(`#should be able to generate and delete a share, manualSync=${mode}`, async function () {
        ...
        const { tssShare: retrievedTSS, tssIndex: retrievedTSSIndex } = await tb.getTSSShare(newFactorKey);
      ...
      // it.only(`#random test, manualSync=${mode}`, async function () {
      //   // nothing
      // });
